...
from models.config import add_dit_config
...
def print_attr_tree(obj, indent=0, visited=None):
    if visited is None:
        visited = set()
    ...
    for k, v in vars(obj).items():
        if isinstance(v, torch.nn.Module):
            ...
            print_attr_tree(v, indent + 4, visited)
            ...
            val_str = str(v)
            if len(val_str) > 40: val_str = val_str[:40] + "..."
