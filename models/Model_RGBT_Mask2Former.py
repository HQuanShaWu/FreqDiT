...
class Model_RGBT_Mask2Former(LightningModule):
    ...
    def build_optimizer(cls, cfg, model):
        ...
        if optimizer_type == "SGD":
            ...
            hidden_gains_biases = [p for n, p in model.named_parameters() \
                                   if p.ndim < 2 and 'embed' not in n and 'head' not in n]
            nonhidden_params = [p for n, p in model.named_parameters() \
                                if 'embed' in n or 'head' in n]
            param_groups = [
                dict(params=hidden_weights, use_muon=True,
                    lr=0.02, weight_decay=0.01),
                dict(params=hidden_gains_biases+nonhidden_params, use_muon=False,
                    lr=cfg.SOLVER.BASE_LR, betas=(0.9, 0.999), weight_decay=0.01),
            ]
            optimizer = maybe_add_full_model_gradient_clipping(MuonWithAuxAdam)(
                param_groups,
            )
        else:
    ...
    def training_step(self, batch_data, batch_idx):
        ...
        if not hasattr(self, 'running_mean'):
            self.running_mean = {k: v.item() for k, v in losses_dict.items()}
        else:
            for k, v in losses_dict.items():
                old_mean = self.running_mean[k]
                new_val = v.item()
                self.running_mean[k] = (old_mean * self.batch_num + new_val) / (self.batch_num + 1)

        if self.batch_num % self.log_freq == 0:
