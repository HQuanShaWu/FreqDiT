...
from .DiT import SegDiTbackbone
import numpy as np
import math
...
class Fusion_Swin_Transformer(nn.Module):
    ...
    def __init__(
        ...
        **kwargs):
        ...
        self.fuse_gates = nn.ParameterList([
                nn.Parameter(torch.zeros(1, 1, self.num_features[i]))
                for i in range(self.num_layers)
            ])
        self.fused_projectors = nn.ModuleList([
                nn.Conv2d(3, self.num_features[i], kernel_size=1)
                for i in range(self.num_layers)
            ])
    ...
    def forward(self, x1, x2, fused):
        ...
        if fused is not None:
            fused = self.channel_projector(fused, 'rgb')
        ...
        if self.model_output_attn:
            ...
            for i_layer in range(self.num_layers):
                ...
                if fused is not None:
                    fused_resized = F.interpolate(fused, size=(H // 2, W // 2) if i_layer < self.num_layers-1 else (H, W), mode='bilinear', align_corners=False)
                    fused_proj = self.fused_projectors[i_layer+1 if i_layer < self.num_layers-1 else i_layer](fused_resized)
                    fused_proj = fused_proj.flatten(2).transpose(1, 2)
                    gate = torch.clamp(self.fuse_gates[i_layer+1 if i_layer < self.num_layers-1 else i_layer], 0, 1)
                    x1 = x1 + gate * fused_proj
                    x2 = x2 + gate * fused_proj
...
class RGBTSwinTransformer(Fusion_Swin_Transformer, Backbone):
    def __init__(self, cfg, input_shape):
        ...
        self.use_dit = cfg.MODEL.SWIN.USE_DIT_BRANCH
        if self.use_dit:
            self.dit_backbone = SegDiTbackbone(cfg)
            state_dict = torch.load(cfg.MODEL.DIT.CKPT_PATH, map_location='cpu')
            self.dit_backbone.load_state_dict(state_dict)
            for param in self.dit_backbone.parameters():
    ...
    def forward(self, x):
        ...
        if self.use_dit:
            fused = self.dit_backbone(x, infer=True)
        ...
        if self.model_output_attn is not True:
            out_features = super().forward(rgb_input, thermal_input, fused)
            align_dict = {}
