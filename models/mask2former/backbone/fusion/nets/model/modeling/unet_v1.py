...
from nets.model.modeling.basic_blocks import ConvBlock
from nets.model.ops import FeaturesConnector
...
class UNetEncoder(nn.Module):
    def __init__(
            ...
            backbone_from, backbone_channels=None, backbone_mode='', _in_channels=4, txt_channel=None
    ):
        ...
        self.block0 = UNetDownBlock(
            ...
            activation=relu,
            pool=True, padding=1, txt_emb_dim=txt_channel
        )
        ...
        for block_i in range(2, depth):
            ...
            self.blocks_connected[f'block{block_i}'] = UNetDownBlock(
                ...
                pool=block_i < depth - 1, txt_emb_dim=txt_channel
            )
            self.block_channels.append(out_channels)

    def forward(self, x, backbone_features, txt_in=None):
        backbone_features = [] if backbone_features is None else backbone_features[::-1]
        outputs = []

        block_input = x
        output, block_input = self.block0(block_input, txt_in)
        outputs.append(output)
        output, block_input = self.block1(block_input, txt_in)
        outputs.append(output)
        ...
        for block_i in range(2, self.depth):
            ...
            output, block_input = block(block_input, txt_in)
            outputs.append(output)
...
class UNetDownBlock(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer, activation, pool, padding, txt_emb_dim=None):
        ...
        self.mlp_txt = nn.Sequential(
            nn.SiLU(),
            nn.Linear(txt_emb_dim, out_channels * 2)
        ) if exists(txt_emb_dim) else None
        self.pooling = nn.MaxPool2d(2, 2) if pool else nn.Identity()

    def forward(self, x, txt_in=None):
        conv_x = self.convs(x)
        if exists(self.mlp_txt) and exists(txt_in):
            txt_emb = self.mlp_txt(txt_in)
            txt_emb = rearrange(txt_emb, 'b c -> b c 1 1')
            txt_scale_shift = txt_emb.chunk(2, dim=1)
            txt_scale, txt_shift = txt_scale_shift
            conv_x = conv_x * (txt_scale + 1) + txt_shift
