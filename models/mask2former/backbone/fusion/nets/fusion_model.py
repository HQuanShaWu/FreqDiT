...
class SwinFusion(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=4, in_chans=3,
        ...
        self.patch_embed_up = nn.ModuleList([
            self.patch_embed
        ])
        ...
        self.patch_unembed_up = []
        # absolute position embedding
        if self.ape:
            self.absolute_pos_embed = nn.Parameter(torch.zeros(1, num_patches, embed_dim))
            trunc_normal_(self.absolute_pos_embed, std=.02)

            self.absolute_pos_embedB = nn.Parameter(torch.zeros(1, num_patches, embed_dim))
            trunc_normal_(self.absolute_pos_embedB, std=.02)
        ...
        for i_layer in range(self.Fusion_num_layers):
            ...
            if i_layer > 0:
                patch_embed = PatchEmbed(
                    img_size=img_size, patch_size=patch_size, in_chans=embed_dim, embed_dim=embed_dim,
                    norm_layer=norm_layer if self.patch_norm else None)
                self.patch_embed_up.append(patch_embed)
            patch_unembed = PatchUnEmbed(
                img_size=img_size, patch_size=patch_size, in_chans=embed_dim, embed_dim=embed_dim,
                norm_layer=norm_layer if self.patch_norm else None)
            self.patch_unembed_up.append(patch_unembed)
            img_size = img_size * 2
    ...
    def forward_features_Ex_B(self, x):
        ...
        x_size = (x.shape[2], x.shape[3])
        x = self.patch_embedB(x)
        if self.ape:
            x = x + self.absolute_pos_embedB
        x = self.pos_drop(x)
    ...
    def forward_features_Fusion(self, x, y):
        ...
        for i, layer in enumerate(self.layers_Fusion):
            ...
            x = self.patch_unembed_up[i](x, x_size)
            y = self.patch_unembed_up[i](y, x_size)
            x = self.up(x)
            y = self.up(y)
            x = self.patch_embed_up[i + 1](x)
            y = self.patch_embed_up[i + 1](y)
            if not isinstance(self.up, nn.Identity):
                x_size = (x_size[0] * 2, x_size[1] * 2)

        x = self.norm_Fusion_A(x)  # B L C
        x = self.patch_unembed_up[-1](x, x_size)

        y = self.norm_Fusion_B(y)  # B L C
        y = self.patch_unembed_up[-1](y, x_size)
        x = torch.cat([x, y], 1)
...
if __name__ == '__main__':
    ...
    x = torch.randn((1, 8, height, width), device='cuda')
    y = torch.randn((1, 8, height, width), device='cuda')
    x = model(x, y)
    print(x.shape)
